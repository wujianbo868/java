...
public class StreamTest2 {
    ...
    public void filter(){
        //获取姓名为Ronaldo的人
        List<Human> nhumans = humans.stream().filter(ex -> ex.getName().equals("Ronaldo")).collect(Collectors.toList());
        System.out.println(nhumans);
        //结果：[Human [name=Ronaldo, sex=male, age=33, id=4], Human [name=Ronaldo, sex=male, age=43, id=5]]

        //获取年龄31的人
        nhumans = humans.stream().filter(it->it.getAge()==31).collect(Collectors.toList());
        System.out.println(nhumans);
        //结果：[Human [name=Sarah, sex=female, age=31, id=1], Human [name=Messi, sex=male, age=31, id=3]]

        //过滤后获取第一个，按照put的顺序取
        Optional<Human> humanOps = humans.stream().filter(it->it.getAge()==31).findFirst();
    ...
    public void aggregate() {
        // 获取最大年龄
        int maxage = humans.stream().collect(Collectors.maxBy(Comparator.comparingInt(Human::getAge))).get().getAge();
        System.out.println(maxage);// 结果：43

        // 获取最小年龄
        int minage = humans.stream().collect(Collectors.minBy(Comparator.comparingInt(Human::getAge))).get().getAge();
        System.out.println(minage);// 结果：12

        // 获取平均年龄
        double avgAge = humans.stream().collect(Collectors.averagingInt(Human::getAge));
        System.out.println(avgAge);// 结果：30
        // 获取总年龄,方法1
        int sumAge = humans.stream().collect(Collectors.summingInt((Human::getAge)));
        System.out.println(sumAge);// 结果：150
        // 获取总年龄,方法2
        sumAge = humans.stream().collect(Collectors.reducing(0, Human::getAge, Integer::sum));
        System.out.println(sumAge);// 结果：150
        // 获取总年龄,方法3
        sumAge = humans.stream().mapToInt(Human::getAge).sum();
        System.out.println(sumAge);// 结果：150
        // 获取总年龄,方法4,并行处理
    ...
    public void sort(){
        ...
        List<Human> nhumans = humans.stream()
                .sorted(Comparator.comparingInt(r->r.getAge())).collect(Collectors.toList());
        System.out.println(nhumans);
        //从小到大排序，实现方式2
        nhumans = humans.stream()
                .sorted(Comparator.comparing(Human::getAge)).collect(Collectors.toList());
        System.out.println(nhumans);
    ...
    public void transfer(){
        ...
        //转成map，key为id,方法1
        Map<Long, Human> map = humans.stream().collect(Collectors.toMap(Human::getId, it->it));
        System.out.println(map);
        //转成map，key为id,方法2
        map = humans.stream().collect(Collectors.toMap(Human::getId, Function.identity()));
        System.out.println(map);
        ...
        try {
            smap = humans.stream().collect(Collectors.toMap(Human::getName, Function.identity()));
        ...
        //抽取name字段
        List<String> names = humans.stream().map(Human::getName).collect(Collectors.toList());
        System.out.println(names);
        //结果：[Sarah, Mary, Messi, Ronaldo, Ronaldo]

        //抽取name字段,去重
        names = humans.stream().map(Human::getName).distinct().collect(Collectors.toList());
        System.out.println(names);
    ...
    public void parallel(){
        ...
        nhumans.stream().forEach(it->rs.append(it.age).append(","));
        System.out.println(rs.toString());
        //结果：12,31,31,33,43,

        //并行处理，结果无序的：
        StringBuffer prs = new StringBuffer();
        nhumans.parallelStream().forEach(it->prs.append(it.age).append(","));
